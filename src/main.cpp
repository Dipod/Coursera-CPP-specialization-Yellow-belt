...
void PrintStats(vector<Person> persons) {
	cout << "Median age = " << ComputeMedianAge(persons.begin(), persons.end())
			<< endl;
	auto it = partition(persons.begin(), persons.end(), [](Person &i) {
	...
	cout << "Median age for males = " << ComputeMedianAge(it, persons.end())
			<< endl;
	auto it2 = partition(persons.begin(), it, [](Person &i) {
		return i.is_employed;
	});
	cout << "Median age for employed females = "
			<< ComputeMedianAge(persons.begin(), it2) << endl;
	cout << "Median age for unemployed females = " << ComputeMedianAge(it2, it)
			<< endl;
	it2 = partition(it, persons.end(), [](Person &i) {
		return i.is_employed;
	});
	cout << "Median age for employed males = " << ComputeMedianAge(it, it2)
			<< endl;
